...
function install_all {
    for ((i=0;i<=$num_nomad_leader-1;i++)); do
        (
            ...
            scp -q -o StrictHostKeychecking=no $OUTPUT_CONSUL/$DATACENTER-client-consul-$i.pem $user@$addr:/tmp/$DATACENTER-client-consul.pem
            scp -q -o StrictHostKeychecking=no $OUTPUT_CONSUL/$DATACENTER-client-consul-$i-key.pem $user@$addr:/tmp/$DATACENTER-client-consul-key.pem
            install "$user" "$addr" "$i"
        ) & 
    done
    wait
...
function main {
    clear_logs
    printf "\nInstalling consul client on nomad leaders...\n"
    install_all
    clean_all
    printf "Done!\n"
}
